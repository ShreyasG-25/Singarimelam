...
<body>
    ...
    <center>
            <h2>View Members List For Events</h2><br>
        </center>
    <!-- header-end -->
    <!-- Postarea-start -->
    <div id="leftbar">
   ...
   <?php
        error_reporting(0);
        $query="SELECT * from book_info WHERE arranged='yes'";
        ...
        while($rows=mysqli_fetch_assoc($res))
        {
            ...
            $dolenum=$num/3;
            $thalanum=$num/3;
            $sql="SELECT memberinfo.name,memberinfo.position,post.date,joined.logs,memberinfo.contact FROM joined INNER JOIN post on post.eid=joined.eid INNER JOIN memberinfo on joined.mid=memberinfo.mid  WHERE memberinfo.position='chende' LIMIT $chendenum";
            $sql2="SELECT memberinfo.name,memberinfo.position,post.date,joined.logs,memberinfo.contact FROM joined INNER JOIN post on post.eid=joined.eid INNER JOIN memberinfo on joined.mid=memberinfo.mid  WHERE memberinfo.position='tutor' LIMIT $dolenum";
            $sql3="SELECT memberinfo.name,memberinfo.position,post.date,joined.logs,memberinfo.contact FROM joined INNER JOIN post on post.eid=joined.eid INNER JOIN memberinfo on joined.mid=memberinfo.mid  WHERE memberinfo.position='Thala' LIMIT $thalanum";

                $result1=mysqli_query($con,$sql);
                if(mysqli_num_rows($result1)>0)
                {
                    echo"<h3><center>Selected Members List for Event ID $eid</center></h3>";
                    echo"<h3><center>Total Artists: $num</center></h3>";
                    echo"<h2>On ".$date."</h2>";
                    echo"<h3>Chende</h3>";
                    ?>
                ...
                if(mysqli_num_rows($result3)>0)
                {
                    ...
                    <br><br>
                    <?php
                    echo"<h4>The following members are assigned to this event and are expected to be present in the team house 1 hour early to departure time.</h4><br><br>"; 
                }
        }
    }
        ?>
