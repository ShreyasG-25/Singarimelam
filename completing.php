...
<body>
	...
	<div id="data">
		<form class="myform" name="idsearch" action="completing.php" method="post">
				<h3>Complete Events</h3>
				<label><b>ID</b></label>
				<input name="id" type="text" class="inputvalues" placeholder="Booking request id" required/><br>
				<input name="submit_btn" type="submit" id="signup_btn" value="Complete"/><br>
		...
		{
			$id =$_POST['id'];
			$querry="SELECT * FROM book_info WHERE id=$id AND accept='yes' AND arranged='yes' AND completed='no'";
                $result1=mysqli_query($con,$querry);
                if(mysqli_num_rows($result1)>0)
                {        
					$query="UPDATE book_info SET completed='yes' WHERE id=$id";
		...
		{
			echo '<script type="text/javascript"> alert("No such arranged event exists!") </script>';
		}
