...
<body>
	...
	<div id="data">
		<form class="myform" name="idsearch" action="completed.php" method="post">
				...
				<a href="upload.php"><input name="accept_btn" type="button" id="accept_btn" value="Add Post"/><br></a>
		...
		{
		$id =$_POST['id'];
		$query="SELECT * FROM book_info WHERE id=$id AND accept='yes' AND arranged='yes' AND completed='yes'";
		...
		$query="SELECT * FROM book_info WHERE accept='yes' AND arranged='yes' AND completed='yes' AND completed='yes'";
		$result=mysqli_query($con,$query);
		if(mysqli_num_rows($result)>0){
			echo"<h3><center>Completed Events</center></h3>";
			while($row=mysqli_fetch_assoc($result)){
				echo"<h4>ID: </h4>" . $row["id"]. "<br>" . "Name:" . $row["name"]. "<br>" . "Email:" . $row["email"]. "<br>" . "Number:" . $row["number"]. "<br>" . "Alternate number:" . $row["alt_number"]. "<br>" . "Start Time:" . $row["start_time"]. "<br>" . "End Time:" . $row["end_time"]. "<br>" . "Date:" . $row["date"]. "<br>" . "Event Type:" . $row["event_type"]. "<br>" . "Per Person Pay:" . $row["pay"]. "<br>" . "Required People:" . $row["total_people"]. "<br>" . "Event Location:" . $row["event_loc"]. "<br>" . "Event Info:" . $row["event_info"]. "<br>" . "Distance in km:" . $row["distance"]. "<br>" . "Expected Pay:" . $row["expected_pay"]. "<br><br><br>";
					...
					echo"<h3><center>No Completed Events found!!</center></h3>";
		}
		?>
	</div>
</body>
