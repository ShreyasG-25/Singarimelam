...
?>
<?php
        $query="SELECT * FROM post ORDER BY eid DESC";
    ...
    <div id="data">
           ...
           <?php if(!empty($posts)): ?>
                ...
                <?php foreach($posts as $post): ?>
                    <h2>On <?php echo$post['date'] ?></h2>
                    <table>

                        <?php
                            echo"<tr>
                                <td>Event type:".$post['event_type']."</td>
                            </tr>
                             <tr>
                                <td>Event info:".$post['event_info']."</td>
                            </tr>
                             <tr>
                                <td>Location:".$post['event_loc']."</td>
                            </tr>
                            <tr>
                                <td>Payment:".$post['pay']."</td>
                            </tr>
                            <tr>
                                <td>Combination:".$post['total_people']."</td>
                            </tr>
                            <tr>
                                <td>Distance in km:".$post['distance']."</td>
                            </tr>
                            <tr>
                                <td>Event start time:".$post['start_time']."</td>
                            </tr>
                            <tr>
                                <td>End time:".$post['end_time']. "</td>
                            </tr> ";
                        ...
                        <a href="join.php?id=<?php echo $post['eid']; ?>" class="register_btn">Apply to join</a></td></tr>
                         <tr><td><?php echo"_______________________________________________________________________________________________________________________________________________________________";?></td></tr>
                    </form>
                    </table>
        ...
        <?php 
            if(isset($_GET['id']))
            {
                $eid=$_GET["id"];
                $mid=$_SESSION["mid"];
                $querry="SELECT * FROM joined WHERE eid=$eid AND mid=$mid";
                $result1=mysqli_query($con,$querry);
                if(mysqli_num_rows($result1)>0){
                    echo '<script type="text/javascript"> alert("You have already joined this event") </script>';
                }
                else
                {
                $sql="INSERT INTO joined(eid,mid,logs) VALUES({$_GET["id"]},{$_SESSION["mid"]},now())";
                // {$_GET["id"]},{$_SESSION["mid"]}
                 $con->query($sql);
               echo '<script type="text/javascript"> alert("Sucess! You have applied to join event") </script>';
