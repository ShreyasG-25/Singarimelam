...
<body>
	...
	<div id="data">
		<form class="myform" name="idsearch" action="pending.php" method="post">
				...
				<a href="arranging.php"><input name="accept_btn" type="button" id="accept_btn" value="Arrange"/><br></a>
		...
		{
		$id =$_POST['id'];
		$query="SELECT * FROM book_info WHERE id=$id AND accept='yes' AND arranged='no'";
		...
		$query="SELECT * FROM book_info WHERE accept='yes' AND arranged='no'";
		$result=mysqli_query($con,$query);
		if(mysqli_num_rows($result)>0){
			echo"<h3><center>Pending Events</center></h3>";
			while($row=mysqli_fetch_assoc($result)){
